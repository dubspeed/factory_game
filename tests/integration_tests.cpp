...
TEST(TwoMachineProductionChain, WithSimpleBelt) {
    ...
    m1->getFirstInput()->addAmount(MAX_STACK_SIZE, Resource::IronOre);
    ...
    w.advanceBy(2000 + 100, [&]() {
        ...
        EXPECT_TRUE(m2->getFirstInput()->isEmpty());
    ...
    w.advanceBy(105333, [&]() {
        EXPECT_EQ(m1->getFirstInput()->getAmount(), 45);
        EXPECT_EQ(m2->getOutputStack(0)->getAmount(), 32);
        EXPECT_EQ(belt->_in_transit_stack.size(), 1);
    });
    // first machine is finished last ingot shipping
    w.advanceBy(90000, [&]() {
       EXPECT_TRUE(m1->getFirstInput()->isEmpty());
        EXPECT_TRUE(m1->getOutputStack(0)->isEmpty());
       EXPECT_EQ(m2->getFirstInput()->getAmount(), 2);
...
TEST(DepositToMachine, WithSimpleBelt) {
    ...
    w.advanceBy(2 * 60 * 1000, [&]() {
        ...
        EXPECT_EQ(m1->getFirstInput()->getAmount(), 59);
    ...
    w.advanceBy(5 * 60 * 1000, [&]() {
        ...
        EXPECT_EQ(m1->getFirstInput()->getAmount(), 100);
